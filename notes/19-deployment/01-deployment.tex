...
First update \texttt{apt}, used for installing Linux packages, so that it has the most up to date package versions:
...
\begin{minted}{bash}
    sudo apt install -y mysql-server git zip acl
\end{minted}

And PHP:

\begin{minted}{bash}
    sudo apt install -y php-fpm php-mysql php-mbstring php-dom
\end{minted}
...
On your GitHub repository, go to clone or download and, as usual, copy the code (ensure it says clone with SSH):
...
Check the page in your browser. Hopefully you'll see the Laravel welcome page.
